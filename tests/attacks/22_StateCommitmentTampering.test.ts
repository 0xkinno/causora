...
describe("Attack Vector 22: Causal Witness with wrong stateCommitment is rejected as INDETERMINATE", function () {
  let f: AttackFixture;
  beforeEach(async () => {
    f = await setupAttackFixture();
  });

  it("Causal Witness with wrong stateCommitment is rejected as INDETERMINATE", async function () {
    const evA = {
      chainKey: 1n,
      blockHeight: 100n,
      txIndex: 0n,

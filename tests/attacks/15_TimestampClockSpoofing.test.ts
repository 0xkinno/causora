...
describe("Attack Vector 15: Rejects timestamps as evidence of cross-chain order", function () {
  let f: AttackFixture;
  beforeEach(async () => {
    f = await setupAttackFixture();
  });

  it("Rejects timestamps as evidence of cross-chain order", async function () {
    const evA = {
      chainKey: 1n,
      blockHeight: 100n,
      txIndex: 0n,

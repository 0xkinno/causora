...
interface ICausoraRegistry {
    ...
    event SourceRegistered(uint64 indexed chainKey, address indexed emitter, SourceKind kind, string description);
    event SourceRemoved(uint64 indexed chainKey, address indexed emitter);
    event EventEvidenceAdmitted(bytes32 indexed queryId, uint64 indexed chainKey, uint64 indexed blockHeight, uint64 txIndex, address emitter, bytes32 eventSig);
    ...
    function computeQueryId(uint64 chainKey, uint64 blockHeight, bytes32 merkleRoot, INativeQueryVerifier.MerkleProofEntry[] calldata siblings) external view returns (bytes32);
